...
export default function Documents() {
  ...
  const partyList = useMemo(
    () =>
      Array.from(new Set(documents.map((d) => d.party))).sort((a, b) =>
  ...
  const columns: Column<DocumentRow>[] = [
    {
      ...
      render: (r) => (
        <span className="tnum">
          {r.docNo ?? "—"}
          {r.system && (
            <span style={{ color: "var(--text-muted)" }}> {r.system}</span>
          )}
        </span>
      ),
    },
    {
      ...
      width: 92,
      value: (r) => r.date,
      render: (r) => <span className="tnum">{date(r.date)}</span>,
    },
    {
      key: "typeLabel",
      header: "Төрөл",
      ...
      render: (r) => (
        <span className="flex items-center gap-1.5">
          <span
            ...
            style={{
              background: typeGroupColor(TYPE_GROUP[r.typeCode] ?? "Бусад"),
            }}
          />
          {r.typeLabel}
          {r.seqNo ? ` ${r.seqNo}` : ""}
    ...
    },
    {
      ...
      width: 150,
      value: (r) => r.category,
      render: (r) => CATEGORY_SHORT[r.category] ?? r.category,
    },
    {
      key: "filename",
      header: "Файл",
      ...
      render: (r) => (
        <span className="block max-w-[430px] truncate" title={r.path}>
          {r.filename}
          {r.isDuplicateFile && <Badge className="ml-1.5">давхардсан</Badge>}
          {r.supersededBy && (
            <Badge className="ml-1.5">{r.supersededBy}-аар солигдсон</Badge>
          )}
          {r.isAmendment && <Badge className="ml-1.5">нэмэлт</Badge>}
        </span>
      ),
    },
    {
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        <StatTile
          label="Шүүлтэд тохирсон"
          value={num(filtered.length)}
          sub={`нийт ${num(meta.fileCount)} баримтаас`}
          accent="var(--series-1)"
        />
        <StatTile
          label="Огноогүй баримт"
          value={num(noDate)}
          sub="файлын нэрэнд огноо байхгүй"
          tone={noDate ? "warning" : undefined}
        />
        <StatTile
          label="Давхардсан файл"
          value={num(dupes)}
          sub="нэг баримт хоёр хавтсанд"
          tone={dupes ? "warning" : undefined}
        />
        <StatTile
          label="Хэмжээ"
          value={bytes(filtered.reduce((s, d) => s + d.sizeBytes, 0))}
          sub={`${new Set(filtered.map((d) => d.party)).size} тал`}
        />
      </div>

      <Card
        title="Баримт бичгийн бүртгэл"
        subtitle="Багана дээр дарж эрэмбэлнэ"
        right={
          <div className="flex flex-wrap items-center gap-2 no-print">
            <SearchBox
              value={q}
              onChange={setQ}
              placeholder="Файл, тал, дугаараар хайх…"
              className="w-56"
            />
            <Select
              value={cat}
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <Card
          title="Ангилал × баримтын төрөл"
          subtitle="Шүүлтийн дараах тоо. Бүдэг өнгө = цөөн, тод = олон баримт"
        >
          <Heatmap
            cols={matrix.types}
            rowsLabels={matrix.cats}
            get={(r, c) => matrix.get(r, c)}
            colHeader={(c) =>
              c
                .replace("санхүүжилтийн тайлан", "санх. тайлан")
        ...
        <Card
          title="Сар бүрийн баримтын урсгал"
          right={
            <Legend
              items={TYPE_GROUP_ORDER.map((g) => ({
                label: g,
                color: typeGroupColor(g),
              }))}
            />
          }
        >
      ...
      <Card
        title="Дугаарлалтын хяналт"
        subtitle="BI бүртгэлийн дугаарууд болон санхүүжилтийн тайлангийн дараалал"
      >
        <div className="grid gap-5 md:grid-cols-2">
          <div>
            <h4 className="mb-1.5 text-xs font-semibold">
              Энэ хавтсанд байхгүй BI дугаар
            </h4>
            <p className="mb-2 text-xs" style={{ color: "var(--text-muted)" }}>
              BI бүртгэл нь төслийн нийт бичиг хэрэг тул эдгээр дугаар өөр
              хавтсанд байж болно. Гэхдээ Хэйва төслийн хавтсанд байхгүй нь
              шалгах жагсаалт болно.
            </p>
            <div className="flex flex-wrap gap-1">
              {meta.registryGapsBI.map((n) => (
                <Badge key={n} className="tnum">
          ...
          <div>
            <h4 className="mb-1.5 text-xs font-semibold">
              Санхүүжилтийн тайлангийн дараалалд байхгүй дугаар
            </h4>
                ...
                >
                  ...
                  <span className="tnum">
                    ...
                    <span style={{ color: "var(--text-muted)" }}>
                      {" "}
                      (IPR-{String(g.max).padStart(2, "0")} хүртэл)
                    </span>
                  </span>
                </li>
              ))}
            ...
            <h4 className="mt-4 mb-1.5 text-xs font-semibold">
              Гэрээ нь архивт байхгүй тайлан
            </h4>
            <div className="flex flex-wrap gap-1">
              {meta.orphanContractKeys.map((k) => (
                <Badge key={k}>{k}</Badge>
              ))}
            </div>
          </div>
