...
export default function Contracts() {
  ...
  const columns: Column<Contract>[] = [
    {
      ...
      render: (c) => (
        <span className="tnum">
          {c.docNo}
          <span style={{ color: "var(--text-muted)" }}> {c.system}</span>
          {c.amendments.length > 0 && (
            <Badge className="ml-1.5">+{c.amendments.length}</Badge>
          )}
        </span>
      ),
    },
    ...
    },
    {
      ...
      render: (c) => (
        <span
          ...
          style={{
            color:
              c.end && c.end < meta.dateMax && (c.paidPercent ?? 0) < 95
                ? "var(--status-critical)"
                : undefined,
          }}
        >
    ...
    },
    {
      ...
      render: (c) =>
        c.value == null ? (
          <Badge>нэгж үнэтэй</Badge>
        ) : (
          <span>
            {c.currency === "MNT"
              ? mnt(c.value)
              : `${num(c.value)} ${c.currency}`}
            {c.valueBasis !== "нийт" && (
              <Badge className="ml-1.5" title={c.valueBasisNote ?? undefined}>
                {c.valueBasis}
              </Badge>
            )}
          </span>
  ...
  const ganttRows: GanttRow[] = filtered
    ...
    .sort((a, b) =>
      (a.start ?? a.signedDate ?? "").localeCompare(
        b.start ?? b.signedDate ?? "",
      ),
    )
    .map((c) => ({
  ...
  const foreignCurrencyTotals = useMemo(() => {
    ...
    for (const c of foreignContracts)
      m.set(c.currency, (m.get(c.currency) ?? 0) + (c.value ?? 0));
    return Array.from(m.entries());
  }, []);

  const amendCount = contracts.reduce((s, c) => s + c.amendments.length, 0);
  const expiring = contracts.filter(
    (c) => c.end && c.end >= meta.dateMax && c.end <= "2026-10-31",
  );
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        ...
        />
        <StatTile
          label="Нийт гэрээний дүн"
          value={`₮${compact(totalContractValue)}`}
          sub={
            foreignCurrencyTotals.length
        ...
        <StatTile
          ...
          value={pct((totalPaid / totalContractValue) * 100)}
          sub={`${mnt(totalPaid)} / ${mnt(totalContractValue)}`}
          accent="var(--series-3)"
        />
        <StatTile
          label="Хугацаа дуусах гэрээ"
        ...
        right={
          <div className="flex flex-wrap items-center gap-2 no-print">
            <SearchBox
              value={q}
              onChange={setQ}
              placeholder="Тал, дугаар, ажлаар хайх…"
              className="w-56"
            />
            <Select
              value={cat}
      ...
      <div className="grid gap-4 xl:grid-cols-[2fr_3fr]">
        <Card
          title="Гэрээний дүнгийн эрэмбэ"
          subtitle="Эхний 12 гэрээ, төгрөгөөр"
        >
          <RankBar
            rows={valueRank}
            format={(v) => `₮${compact(v)}`}
        ...
        <Card
          title="Гэрээний хугацаа ба санхүүжилтийн гүйцэтгэл"
      ...
      {unpaidContracts.length > 0 && (
        <Card
          title="Санхүүжилтийн тайлан хараахан гараагүй гэрээ"
  ...
  return (
    ...
    >
      ...
      <dl className="mt-3 grid gap-x-6 gap-y-2 text-xs sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Field
          k="Хугацаа"
          v={`${date(contract.start)} – ${date(contract.end)}${contract.periodSource !== "гэрээ" ? " *" : ""}`}
        />
        <Field
          k="Үндсэн дүн"
          v={
            contract.baseValue == null
              ? "—"
              : contract.currency === "MNT"
                ? mnt(contract.baseValue)
                : `${num(contract.baseValue)} ${contract.currency}`
          }
        />
        <Field
          k="Нийт дүн"
          v={
            contract.value == null
              ...
              : contract.currency === "MNT"
                ? mnt(contract.value)
                : `${num(contract.value)} ${contract.currency}`
          }
        />
        <Field
          k="НӨАТ"
          v={
            contract.vatIncluded == null
              ? "—"
        ...
        {contract.valueBasis !== "нийт" && (
          <Field
            k="Дүнгийн үндэслэл"
            v={`${contract.valueBasis} — ${contract.valueBasisNote ?? ""}`}
            span
          />
        )}
        <Field
        ...
        <Field
          k="Санхүүжсэн"
          v={`${mnt(contract.paid)} (${contract.paidPercent != null ? pct(contract.paidPercent) : "—"})`}
        />
      </dl>
      ...
      {pays.length > 0 && (
        <div className="mt-4">
          <h5 className="mb-1.5 text-xs font-semibold">
            Санхүүжилтийн тайлангууд ({pays.length})
          </h5>
                ...
                {pays.map((p) => (
                  <tr key={p.id}>
                    <td className="strong">
                      ...
                      {p.supersededBy && (
                        <Badge className="ml-1.5">хассан</Badge>
                      )}
                    </td>
                    ...
                    <td
                      ...
                      style={{
                        textAlign: "right",
                        color: p.supersededBy
                          ? "var(--text-muted)"
                          : "var(--text-primary)",
                        textDecoration: p.supersededBy
                          ? "line-through"
                          : undefined,
                      }}
                    >
      ...
      {contract.periodSource !== "гэрээ" && (
        <p className="mt-3 text-xs" style={{ color: "var(--text-muted)" }}>
          * Гэрээний хугацааг гэрээнд тодорхой заагаагүй тул санхүүжилтийн
          тайлангийн «Гэрээний хугацаа» мөрнөөс нөхөж авав.
        </p>
      )}
      {contract.notes && (
        <p
...
function Field({ k, v, span }: { k: string; v: string; span?: boolean }) {
  return (
    <div className={span ? "sm:col-span-2 lg:col-span-4" : undefined}>
      ...
      <dd className="tnum mt-0.5" style={{ color: "var(--text-primary)" }}>
        {v}
      </dd>
    </div>
  );
