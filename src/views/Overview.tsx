...
export default function Overview() {
  ...
  const riskTone = {
    ...
    Дунд: "var(--status-warning)",
    Бага: "var(--status-good)",
  };
  const topRisk = partyRisk.slice(0, 5).map((r) => ({
    ...
    sub:
      `${r.tier} эрсдэл · ${r.qualityCount} зөрчил · ${r.overdueCount} хугацаа хэтэрсэн гэрээ` +
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        ...
        />
        <StatTile
          label="Санхүүжсэн дүн"
          value={`₮${compact(totalPaid)}`}
          sub={`${countedPayments.length} тайлан · гэрээний ${pct(paidPct)}`}
          accent="var(--series-2)"
        />
        <StatTile
          label="Үлдэгдэл санхүүжилт"
          value={`₮${compact(remaining)}`}
          sub={`${unpaidContracts.length} гэрээнд тайлан хараахан алга`}
          accent="var(--series-4)"
        />
        <StatTile
          label="Баримт бичиг"
          value={num(meta.fileCount)}
      ...
      <div className="grid gap-4 xl:grid-cols-[3fr_2fr]">
        ...
        <Card
          title="Гэрээний дүнгээр эхний топ 10"
          subtitle="Өнгө нь ангиллыг заана · доорх тоо нь санхүүжсэн хувь"
        >
          <RankBar
            rows={topParties}
            format={(v) => `₮${compact(v)}`}
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        ...
        <Card title="Төслийн эрсдэлийн товч" subtitle="Анхаарал шаардсан зүйлс">
          <ul className="space-y-2.5 text-sm">
            ...
            <RiskRow
              tone="good"
              label="RFI хариу өгөх дундаж хугацаа"
      ...
      <Card
        title="Гэрээний хугацааны гүйцэтгэл"
...
function RiskRow({
  ...
  return (
    <li
      className="flex items-start gap-2.5 border-b pb-2.5 last:border-0"
