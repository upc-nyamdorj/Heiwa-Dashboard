...
import { audit, coverage, spotCheck, meta, contracts, payments } from '@/lib/data';
...
export default function Audit() {
  ...
  const filtered = useMemo(
    () => (sev === ALL ? audit : audit.filter((a) => a.sev === sev)), [sev]);
  ...
  const valuePct = (coverage.contractsWithValue / coverage.contractsTotal) * 100;
  const amountPct = (coverage.paymentsWithAmount / coverage.paymentsTotal) * 100;

  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        <StatTile label="Ноцтой зөрчил" value={num(counts['ӨНДӨР'] ?? 0)}
          sub="Тоо буруу байх магадлалтай" tone={(counts['ӨНДӨР'] ?? 0) > 0 ? 'critical' : 'good'} />
        <StatTile label="Шалгах шаардлагатай" value={num((counts['ДУНД'] ?? 0) + (counts['БАГА'] ?? 0))}
          sub="Ихэвчлэн эх баримтын өөрийнх нь зөрүү" tone="warning" />
        <StatTile label="Гэрээний дүн уншигдсан"
          value={`${coverage.contractsWithValue} / ${coverage.contractsTotal}`}
          sub={`${pct(valuePct, 0)} · үлдсэн нь нэгж үнэт буюу дүнгүй гэрээ`}
          accent="var(--series-1)" />
        <StatTile label="Тайлангийн дүн уншигдсан"
      ...
      <Card title="Өгөгдөл хэрхэн шалгагдсан бэ"
        subtitle="Гурван түвшний шалгалт — тус бүр өөр төрлийн алдаа барьдаг">
        <ol className="space-y-3 text-sm">
          <Step n={1} title="Хөндлөнгийн дахин унших (12 баримт)"
          ...
          <Step n={2} title="Дотоод логикийн шалгалт (доорх жагсаалт)"
          ...
          <Step n={3} title="Арифметикийн тулгалт"
      ...
      <Card title="Дотоод шалгалтын үр дүн"
            ...
            <div key={check}>
              <div className="flex items-baseline gap-2">
                <span className="flex h-4 w-4 shrink-0 items-center justify-center rounded-full text-[10px] font-bold"
                  style={{ background: SEV_COLOR[rows[0].sev], color: '#fff' }} aria-hidden>
                  {SEV_ICON[rows[0].sev]}
                </span>
              ...
              {CHECK_RULE[check] && (
                <p className="mt-1 mb-1.5 pl-6 text-xs leading-relaxed"
                  style={{ color: 'var(--text-muted)' }}>{CHECK_RULE[check]}</p>
              )}
              <ul className="pl-6">
                {rows.map((r, i) => (
                  <li key={i} className="flex flex-wrap items-baseline gap-x-3 border-b py-1.5 text-xs last:border-0"
                    style={{ borderColor: 'var(--grid)' }}>
                    <span className="min-w-[220px] font-medium" style={{ color: 'var(--text-primary)' }}>
                      {r.subject}
                    </span>
                    <span className="flex-1" style={{ color: 'var(--text-secondary)' }}>{r.detail}</span>
                  </li>
                ))}
              </ul>
            </div>
        ...
        <Legend className="mt-4" items={[
          { label: 'ӨНДӨР — тоо буруу байх магадлалтай', color: SEV_COLOR['ӨНДӨР'] },
          { label: 'ДУНД — тулгах шаардлагатай', color: SEV_COLOR['ДУНД'] },
          { label: 'БАГА — тайлбартай зөрүү', color: SEV_COLOR['БАГА'] },
          { label: 'МЭДЭЭЛЭЛ — мэдэж байх зүйл', color: SEV_COLOR['МЭДЭЭЛЭЛ'] },
        ]} />
      </Card>

      <Card title="Гараар шалгах жагсаалт"
          ...
          <table className="grid">
            <thead>
              <tr>
                ...
                <th style={{ width: 130, textAlign: 'right' }}>Санд байгаа дүн</th>
              ...
              {spotCheck.map((s, i) => (
                <tr key={i}>
                  <td>
                    ...
                    {s.flagged && <span className="chip ml-1" style={{
                      borderColor: 'var(--status-warning)', color: 'var(--status-warning)',
                    }}>шалгалтад орсон</span>}
                  </td>
                  <td className="strong">{s.label}</td>
                  <td className="tnum" style={{ textAlign: 'right' }}>
                    {s.currency === 'MNT' ? mnt(s.value) : `${num(s.value)} ${s.currency}`}
                  </td>
                  <td style={{ color: 'var(--text-muted)' }}>{s.look}</td>
                  <td>
                    <span className="block max-w-[420px] truncate" title={s.path}>{s.path}</span>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <Card title="Юуг итгэлтэй хэлж болох вэ"
          subtitle="Талбар бүрийн эх сурвалж, найдвартай байдал">
          <table className="grid">
            <thead>
              <tr><th>Талбар</th><th style={{ width: 150 }}>Эх сурвалж</th><th style={{ width: 90 }}>Хамрах</th></tr>
            </thead>
            <tbody>
              <Row f="Баримтын дугаар, төрөл, огноо, тал"
                s="Файлын нэр" c={`${coverage.documentsTotal} / ${coverage.documentsTotal}`} good />
              <Row f="Гэрээний дүн, валют, НӨАТ"
                s="PDF-ийн зураг" c={`${coverage.contractsWithValue} / ${coverage.contractsTotal}`} />
              <Row f="Санхүүжилтийн дүн"
                s="PDF-ийн зураг" c={`${coverage.paymentsWithAmount} / ${coverage.paymentsTotal}`} good />
              <Row f="Гэрээний хугацаа"
                s="PDF + тайлан" c={`${coverage.contractsWithPeriod} / ${coverage.contractsTotal}`} />
              <Row f="Ажил гүйцэтгэсэн хугацаа"
                s="PDF-ийн зураг" c={`${coverage.paymentsWithPeriod} / ${coverage.paymentsTotal}`} />
              <Row f="Урьдчилгаа %, баталгааны суутгал %"
                s="PDF-ийн зураг"
                c={`${contracts.filter((c) => c.advancePercent != null).length} / ${coverage.contractsTotal}`} weak />
              <Row f="Ажлын дэлгэрэнгүй задаргаа"
                s="Уншаагүй" c="0 / —" weak />
            </tbody>
          </table>
          <p className="mt-3 text-xs leading-relaxed" style={{ color: 'var(--text-muted)' }}>
            <strong style={{ color: 'var(--text-primary)' }}>Файлын нэр</strong> — бүрэн
            найдвартай, машин уншсан текст.{' '}
            <strong style={{ color: 'var(--text-primary)' }}>PDF-ийн зураг</strong> — сканнердсан
            хуудсыг дүрсээр уншсан; тоонууд тод хэвлэгдсэн тул найдвартай ч гараар
            шалгах боломжтой. <strong style={{ color: 'var(--text-primary)' }}>Гар бичмэл</strong> —
            тоон утга болгож аваагүй, зөвхөн тайлбарт тэмдэглэсэн.
          </p>
        </Card>

        <Card title="Дахин боловсруулах"
          ...
          <ol className="space-y-2.5 text-xs leading-relaxed"
            ...
            <li>
              ...
              Ганц нэг тоо буруу бол <code>src/data/heiwa.json</code> дотор шууд засаад
              <code> npm run build</code> ажиллуулна. Бүх нийлбэр, хувь автоматаар дахин бодогдоно.
            </li>
            <li>
              <span className="font-medium" style={{ color: 'var(--text-primary)' }}>
                2. Шинэ баримт нэмэгдвэл.
              </span>{' '}
              Файлын нэрний бүтэц (дугаар · систем · төрөл · тал · огноо) хадгалагдсан
              байвал бүртгэл автоматаар барагдана; дүн нь PDF-ээс дахин уншигдах хэрэгтэй.
            </li>
            <li>
              <span className="font-medium" style={{ color: 'var(--text-primary)' }}>
          ...
          <div className="mt-4 rounded-lg border p-3 text-xs"
            ...
            <p className="mt-1 leading-relaxed" style={{ color: 'var(--text-secondary)' }}>
              Санхүүжилтийн {coverage.paymentsWithAmount}/{coverage.paymentsTotal} тайлангийн
              дүн уншигдсан, нийт ₮{compact(payments.filter((p) => p.counted)
                .reduce((s, p) => s + (p.amount ?? 0), 0))}. Ноцтой зөрчил
              {(counts['ӨНДӨР'] ?? 0) === 0 ? ' үлдээгүй' : ` ${counts['ӨНДӨР']} байна`}.
              Үлдсэн тэмдэглэлүүд нь эх баримтын өөрийнх нь зөрүү (хугацаа сунгасан,
              дугаар өөр бичсэн) бөгөөд тус бүрд нь тайлбар хавсаргасан.
            </p>
          </div>
        </Card>
      </div>
  ...
  return (
    <li className="flex gap-3">
      <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-xs font-semibold"
        style={{ background: 'var(--surface-2)', color: 'var(--text-primary)' }}>{n}</span>
      <div>
        <div className="font-medium">{title}</div>
        <p className="mt-0.5 text-xs leading-relaxed" style={{ color: 'var(--text-secondary)' }}>{body}</p>
      </div>
    </li>
  );
}

function Row({ f, s, c, good, weak }: {
  f: string; s: string; c: string; good?: boolean; weak?: boolean;
}) {
  return (
    <tr>
      ...
      <td className="tnum" style={{
        color: good ? 'var(--success-text)' : weak ? 'var(--status-warning)' : undefined,
      }}>{c}</td>
    </tr>
  );
}
