...
export default function Drawings() {
  ...
  const byCompany = useMemo(() => {
    const m = new Map<
      string,
      { pages: number; sets: number; pending: number }
    >();
    for (const d of drawings) {
      if (!m.has(d.company))
        m.set(d.company, { pages: 0, sets: 0, pending: 0 });
  ...
  const noOriginal = drawings.filter(
    (d) => !d.hasOriginal && (d.hasCopy || d.hasDigital),
  );
  ...
  const columns: Column<Drawing>[] = [
    ...
    },
    {
      ...
      header: "Хувь",
      width: 150,
      value: (d) => (d.hasOriginal ? 1 : 0),
      render: (d) => (
        <span className="flex flex-wrap gap-1">
          {d.hasOriginal && <Badge>эх</Badge>}
          {d.hasCopy && <Badge>хуулбар</Badge>}
          {d.hasDigital && <Badge>файл</Badge>}
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        <StatTile
          label="Нийт хуудас"
          value={num(totalDrawingPages)}
          sub={`${drawings.length} багц · ${drawingCompanies.length} зохиогч`}
          accent="var(--series-1)"
        />
        <StatTile
          label="Эх хувьтай"
          value={`${original} / ${drawings.length}`}
          sub={`${digital} багц файлтай`}
          accent="var(--series-3)"
        />
        <StatTile
          label="Дуусаагүй / дутуу"
          value={num(drawingsPending.length)}
          sub={
            drawingsPending
              .map((d) => d.drawing)
              .filter(Boolean)
        ...
        <StatTile
          label="Эх хувь дутуу"
          value={num(noOriginal.length)}
          sub={
            Array.from(new Set(noOriginal.map((d) => d.company))).join(", ") ||
            "—"
          }
        ...
        right={
          <div className="flex flex-wrap items-center gap-2 no-print">
            <SearchBox
              value={q}
              onChange={setQ}
              placeholder="Зохиогч, шифр, блокоор хайх…"
              className="w-56"
            />
            <Toggles
              value={status}
      ...
      <Card
        title="Зураг зохиогчидтой холбоотой баримт"
        subtitle={`${designDocs.length} баримт — гэрээ, RFI, албан бичиг`}
      >
        <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
          {Array.from(new Set(designDocs.map((d) => d.party)))
            .filter((p) => p !== "—")
            .map((party) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="mt-1.5 flex flex-wrap gap-1">
                    {Array.from(new Set(rows.map((r) => r.typeLabel))).map(
                      (t) => (
                        <Badge key={t}>{t}</Badge>
                      ),
                    )}
        ...
        <p className="mt-3 text-xs" style={{ color: "var(--text-muted)" }}>
          Зургийн бүртгэлд ЭльктроСеть Проект, Арбэл Алтай, Элекбий, Мип дизайн,
          Арт трасс, Хос ногоолин, Номин трейдинг зэрэг зохиогч бүртгэлтэй ч
          тэдгээрийн гэрээ, ажил хүлээлцсэн акт нь энэ хавтсанд байхгүй байна.
        </p>
      </Card>
    </div>
  );
