...
export function barPath(
  ...
  const rad = Math.max(0, Math.min(r, dir === 'up' ? h : w, (dir === 'up' ? w : h) / 2));
  if (dir === 'up') {
    if (h <= 0) return '';
...
export interface TipState { x: number; y: number; node: React.ReactNode }
...
export interface LegendItem { label: string; color: string; hint?: string }

export function Legend({ items, className = '' }: { items: LegendItem[]; className?: string }) {
  return (
    <div className={`flex flex-wrap items-center gap-x-4 gap-y-1.5 ${className}`}>
      {items.map((it) => (
        <span key={it.label} className="flex items-center gap-1.5 text-xs"
          style={{ color: 'var(--text-secondary)' }}>
          <span className="inline-block h-2.5 w-2.5 rounded-[3px]" style={{ background: it.color }} />
          {it.label}
          {it.hint && <span style={{ color: 'var(--text-muted)' }}>{it.hint}</span>}
        </span>
      ))}
    </div>
  );
...
export function StatTile({
  ...
  const toneColor = tone
    ? { good: 'var(--success-text)', warning: 'var(--status-warning)', critical: 'var(--status-critical)' }[tone]
    : undefined;
  return (
    <div className="card px-4 py-3.5">
      ...
      <div className="mt-1.5 text-2xl leading-tight font-semibold"
